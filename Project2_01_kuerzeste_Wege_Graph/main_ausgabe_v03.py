"""
3rd version of main_ausgabe
main reason:
- adapted on new Graph_v07
"""

from Graph_v07 import Graph
...
class StreetMap(QtWidgets.QWidget):
    def __init__(self):
        ...
        # landmark test
        self.our_graph.set_landmark_nodes_random(5)
        self.landmarks = self.our_graph.get_landmark_nodes()
        print("CREATING LANDMARKS")
        self.our_graph.calculate_paths_for_landmark_nodes()
        #print("HIER:", landmarks)
    ...
    def paintEvent(self, paint_event):
        ...
        if self.P != -1 and self.our_graph.get_distance_of_path(self.P) != -1 :
        ...
        # show landmarks
        self.set_pen(pen, painter, '#FF00FF', 10)
        for landmark in self.landmarks:
            self.show_specific_node(painter, landmark[0])
    ...
    def calculate_path_nodes(self):
        if self.start != -1 and self.destination != -1:
            self.P = self.our_graph.berechne_den_weg_improved(self.start, self.destination)
