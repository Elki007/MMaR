...
class GameWindow(qw.QLabel):
    def __init__(self, parent):
        ...
        for j in range(self.parent.width()):
            ...
            for i in range(self.parent.height()):
                if self.surface_true_false[i][j]:
                    self.painter0_generic_surface.setPen(qg.QColor(self.surf_color_R, self.surf_color_G, self.surf_color_B))
        ...
        for i in range(len(self.player_names)):
            self.players.append(Player(self, qc.Qt.darkGreen, qg.QColor(104, 156, 56), self.player_names[i], len(self.players)))
        ...
        self.r_start = datetime.now().time()  # Nicht notwendig, wird wohl später überschrieben
    ...
    def movement(self):
        if self.movement_right:
            ...
            if self.players[self.current_player].x_player + self.players[self.current_player].tank_width != self.parent.width():
                # if a slope is not vertical
                if self.ebene2_surface_image.pixel(self.players[self.current_player].x_player + 4, self.players[self.current_player].y_player - 5) == 0:
...
class Hit:
    ...
    def paint(self):
        ...
        if self.crit:
            test = "Critical! "
            self.parent.painter4_interface.setFont(qg.QFont('CorpoS', 14, weight=qg.QFont.Bold, italic=qg.QFont.StyleItalic))
        else:
            test = ""
...
class Bullet:
    def __init__(self, parent, player_number):
        ...
        self.x_bullet = parent.players[self.player_number].x_player + 1.2 * parent.players[self.player_number].gun_vector[0]
        self.y_bullet = parent.players[self.player_number].y_player + 1.2 * parent.players[self.player_number].gun_vector[1]
            ...
            for each in self.parent.players:
                if abs(each.x_player - x_bullet) < each.tank_width_rotated / 2 + self.explosion / 2:
                    if abs(each.y_player - y_bullet) < each.tank_height_rotated / 2 + self.explosion / 2:
                        koeff = 1 - (abs(each.x_player - x_bullet) - each.tank_width_rotated / 2) / (self.explosion / 2)  # dmg.koeff
                        if koeff > 1: koeff = 1
                        if self.criticat:
                            ...
                            each.hp -= dmg
                        # print(dmg)
                        self.parent.hits.append(Hit(self.parent, each.x_player, each.y_player, dmg, koeff, self.criticat))
...
class Player:
    ...
    def start_position_x(self):
        if len(self.parent.players) > 0:
            ...
            while position_not_found:
                ...
                for each in self.parent.players:
                    if (each.x_player - self.start_position_distance) < start_position < (each.x_player + self.start_position_distance):
                        position_not_found = True
                        break
        else:
            start_position = random.randint(self.tank_width, self.parent.parent.width() - self.tank_width)
    ...
    # calculation of y-coordinate for start position
    ...
    def paint(self):
        ...
        self.parent.painter3_object.drawPixmap(self.x_player - self.tank_width / 2, self.y_player - self.tank_height / 2 + 1, tank_pixmap)
        ...
        self.parent.painter4_interface.setFont(qg.QFont('Helvetica', 12))
        self.parent.painter4_interface.drawText(self.x_player - self.tank_width / 2, self.y_player - self.tank_height, self.name)
        ...
        if self.hp > 0:
            ...
            self.parent.painter4_interface.drawRect(self.x_player - self.tank_width / 2, self.y_player - self.tank_height + 2, self.tank_width * percent, 2)
        else:
            self.parent.painter4_interface.setPen(qg.QPen(qc.Qt.red, 2, qc.Qt.SolidLine))
            self.parent.painter4_interface.setFont(qg.QFont('Helvetica', 12))
            self.parent.painter4_interface.drawText(self.x_player - self.tank_width / 2, self.y_player + self.tank_height + 12, "dead")
        ...
        self.parent.painter3_object.drawLine(self.x_player, self.y_player, self.x_player + self.gun_vector[0], self.y_player + self.gun_vector[1])
        ...
        if self.timer > 0:
            ...
            for i in range(len(self.parts)-1, -1, -1):
                ...
                if self.parent.parent.ebene3_object_image.pixel(self.parts[i].x_part, self.parts[i].y_part) != 0:
                    for each in self.parent.parent.players:
                        ...
                        if abs(each.x_player - x_parts) <= each.tank_width_rotated:
                            if abs(each.y_player - y_parts) <= each.tank_height_rotated:
                                ...
                                each.hp -= dmg
                                self.parent.parent.hits.append(Hit(self.parent.parent, each.x_player, each.y_player, dmg, koeff, critical))
                    ...
                    if self.parent.parent.ebene2_surface_image.pixel(self.parts[i].x_part - self.parts[i].part_vector[0], self.parts[i].y_part - self.parts[i].part_vector[1]) == 0:
                        while self.parent.parent.ebene2_surface_image.pixel(self.parts[i].x_part, self.parts[i].y_part) != 0:
...
class Part:
    def __init__(self, parent, smoke):
        ...
        if self.smoke == 1:
            ...
            self.y_part = random.randint(round(self.parent.y_explosion), round(
                self.parent.y_explosion + math.sqrt(abs((self.parent.parent.explosion / 2) ** 2 - (self.x_part - parent.x_explosion) ** 2))))
    ...
    def paint(self):
        ...
        if self.smoke == 1:
            ...
            self.part_vector[0] += self.weight * 2 * random.uniform(-1, 1)
            ...
            self.parent.parent.parent.painter5_smoke.drawEllipse(self.x_part - self.smoke_size / 2, self.y_part - self.smoke_size / 2, self.smoke_size, self.smoke_size)
        else:
            self.x_part += self.part_vector[0]
            self.y_part += self.part_vector[1]
            self.part_vector[1] += self.weight * self.gravity
            ...
            self.parent.parent.parent.painter1_background.setPen(qg.QPen(self.color, self.explosion_size, qc.Qt.SolidLine))
