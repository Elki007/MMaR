...
class GameWindow(qw.QLabel):
    ...
    def Run(self):
        try:
            ...
            # movement of tank dependent on status of self.movement_left / *_right
            self.movement()
    ...
    # Pressing and releasing the key switches the status of self.movement_left and *_right between True and False
    def keyPressEvent(self, event):
        if event.key() == qc.Qt.Key_D and not event.isAutoRepeat():
            self.movement_right = True
        elif event.key() == qc.Qt.Key_A and not event.isAutoRepeat():
