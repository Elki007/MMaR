...
class SceneWindow(qw.QLabel):
    def __init__(self, parent):
        ...
        # fill transparent
        self.ebene_main.fill(qg.QColor(0, 0, 0))
    ...
    def init_ui(self):
        ...
        side_front = Polygon([ftl, ftr, fbr, fbl], qg.QColor(0, 0, 200))
        side_back = Polygon([rtl, rtr, rbr, rbl], qg.QColor(200, 0, 0))
        side_left = Polygon([ftl, rtl, rbl, fbl], qg.QColor(0, 200, 0))
        side_right = Polygon([ftr, rtr, rbr, fbr], qg.QColor(0, 200, 200))
        side_top = Polygon([ftl, ftr, rtr, rtl], qg.QColor(200, 0, 200))
        side_down = Polygon([rbl, rbr, fbr, fbl], qg.QColor(200, 200, 200))
        ...
        self.cube = Object3D(self, [side_front, side_back, side_left, side_right, side_top, side_down], x=500, y=300, z=0, scale=7)
        self.cube2 = Object3D(self, [side_front, side_back, side_left, side_right, side_top, side_down], x=600, y=300, z=1, scale=5)

        self.objects_list.append(self.cube)
        self.objects_list.append(self.cube2)

        self.timer()

    def update_layers(self):
        self.ebene_main.fill(qg.QColor(10, 10, 10))
    ...
    def draw_test_object(self):
        ...
        for i in range(len(self.objects_list)):
            depth_array.append([i,self.objects_list[i].z])

        depth_array.sort(key=operator.itemgetter(1), reverse=True)

        for i in range(len(depth_array)):
            self.objects_list[depth_array[i][0]].draw_perspective(self.objects_painter, 300, 40, angleX=self.angleX, angleY=45, shader=True)
